...
int rslt;
  ...
  sbio = BIO_new_socket((int)skt, BIO_NOCLOSE);
...
// The command and arguments are combined (provided the arg pointer is not zero) and the command is
// sent. Clears the response buffer and returns result of send the command
...
int    rslt;
  ...
  if (rslt) {blk.n += (int)n;  return SSLtrue;}
