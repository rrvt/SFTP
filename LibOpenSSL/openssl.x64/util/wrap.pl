...
BEGIN {
    ...
    require 'D:/Sources/LibOpenSSL/openssl.x64/util/perl/OpenSSL/Util.pm';
    OpenSSL::Util->import();
}
...
if ($ARGV[0] eq '-fips') {
    $std_openssl_conf = 'D:/Sources/LibOpenSSL/openssl.x64/test/fips-and-base.cnf';
    shift;
...
if ($ARGV[0] eq '-jitter') {
    $std_openssl_conf = 'D:/Sources/LibOpenSSL/openssl.x64/test/default-and-jitter.cnf';
    shift;
