...
int        arWidth{0};                // Width of output to a text file
double     tabFactor{0};              // n = tab * tabFactor where tab is an integer and n is the
...
private:
  ...
  NotePad& append(const CString& cs);
...
class NPIter : public ListIter {
public:

  NPIter(NotePad& np) : ListIter(np.noteList) { }
 ~NPIter() { }

  Note* operator() ()    {return (Note*) (*(ListIter*)this)();}     // Initialize scan
  Note* operator++ (int) {return (Note*) (*(ListIter*)this)++;}     // Get next node

private:

  NPIter() : ListIter(*(NoteList*)0) { }
  };
...
Note* note{0};
 ...
 ~NPItem() { }
  ...
  void  operator= (Note* nt) {note = nt;   itemIndex = 0;}

  int   operator() ()    {return   itemIndex = 0;}
  int   operator++ (int) {return ++itemIndex;}
  int   operator-- (int) {return --itemIndex;}
  int   cur()            {return   itemIndex;}
  bool  isFin()          {return   itemIndex >= 21;}
  };
